...
TEST_OUTPUT=$(PI_CODING_AGENT_DIR="$AGENT_DIR" pi -p --provider "${PROVIDER:-openai}" --model "${MODEL:-gemini-3.6-flash-high}"${API_KEY:+ --api-key "$API_KEY"} --thinking low --no-session --no-context-files --approve "Reply with exactly: OK" </dev/null 2>&1)
TEST_EXIT=$?
