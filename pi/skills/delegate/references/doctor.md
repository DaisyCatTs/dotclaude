...
# Only default provider to openai when a custom BASE_URL is set; otherwise omit
# --provider so pi uses its own default when PROVIDER is unset.
PROBE_PROVIDER="${PROVIDER:-${BASE_URL:+openai}}"
TEST_OUTPUT=$(PI_CODING_AGENT_DIR="$AGENT_DIR" pi -p ${PROBE_PROVIDER:+--provider "$PROBE_PROVIDER"} --model "${MODEL:-gemini-3.6-flash-high}"${API_KEY:+ --api-key "$API_KEY"} --thinking low --no-session --no-context-files --approve "Reply with exactly: OK" </dev/null 2>&1)
TEST_EXIT=$?
