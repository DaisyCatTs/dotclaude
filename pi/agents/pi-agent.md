...
4. **Resolve the agent directory** — pi reads endpoints/credentials/sessions from an "agent dir" (default `~/.pi/agent`). It is overridable via the `PI_CODING_AGENT_DIR` env var, which `getAgentDir()` honors. Use `AGENT_DIR` from the caller, else `$PI_CODING_AGENT_DIR`, else `~/.pi/agent`:
   ```bash
